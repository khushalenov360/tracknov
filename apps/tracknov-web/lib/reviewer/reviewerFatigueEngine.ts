...
export class ReviewerFatigueEngine {
  /**
   * Evaluates reviewer actions to detect fatigue states
   */
  static analyzeFatigue(logs: ReviewActionLog[]): FatigueAnalysis {
    if (logs.length === 0) {
      return {
        fatigueScore: 0,
        level: "NORMAL",
    ...
    let level: FatigueAnalysis["level"] = "NORMAL";
    if (fatigueScore >= 80) level = "CRITICAL";
    else if (fatigueScore >= 60) level = "HIGH";
    else if (fatigueScore >= 35) level = "MODERATE";
