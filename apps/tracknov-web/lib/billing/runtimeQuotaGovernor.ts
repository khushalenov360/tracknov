export interface UsageQuotas {
  ...
  uploadsCount: number;
  uploadsLimit: number;
  ocrProcessedBytes: number;
  ocrProcessedLimit: number;
  exportsCount: number;
  exportsLimit: number;
  reviewerSeatsUsed: number;
  reviewerSeatsLimit: number;
...
export interface QuotaEvaluationResult {
  allowed: boolean;
  degradeProcessing: boolean;
  alertTriggered: boolean;
  errorMessage: string | null;
  degradationSuggestion: string | null;
  activeQuotaLevel: "NORMAL" | "DEGRADED" | "BLOCKED";
}

export class RuntimeQuotaGovernor {
  /**
   * Enforces infrastructure cost parameters gracefully without crashing active user submission channels
   */
  static checkQuota(
    category: "AI_TOKEN" | "UPLOAD" | "OCR" | "EXPORT" | "SEATS" | "STORAGE",
    requestedAmount: number,
    currentUsage: UsageQuotas
  ): QuotaEvaluationResult {
    let allowed = true;
    let degradeProcessing = false;
    let alertTriggered = false;
    let errorMessage: string | null = null;
    let degradationSuggestion: string | null = null;
    let activeQuotaLevel: QuotaEvaluationResult["activeQuotaLevel"] = "NORMAL";
