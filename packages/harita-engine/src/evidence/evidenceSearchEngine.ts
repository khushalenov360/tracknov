export interface ReusableEvidenceItem {
  id: string;
  tenantId: string;
  name: string;
  type: string;
  ...
  approvalLineage: string;
  frameworkCompatibility: string[]; // LEED, GRIHA, IGBC
  textExcerpt: string;
}

export class EvidenceSearchEngine {
  // Mock indexed evidence database (strictly tenant-isolated during runtime lookup)
  private static mockDatabase: ReusableEvidenceItem[] = [
    {
      id: "EVID-001",
   ...
   */
  static search(tenantId: string, query: string): ReusableEvidenceItem[] {
