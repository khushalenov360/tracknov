  ...
  public static evaluate(query: string, runtimeContext: any, graphContext: any): ReasoningOutput {
    ...
    if (!creditMatch) {
      if (runtimeContext.project) {
        return {
          directAnswer: "I have analyzed the project context for recommendations.",
      ...
      return {
        directAnswer: "I cannot determine which credit you need a recommendation for.",
    ...
    if (actions.length === 0) {
      return {
        directAnswer: `No immediate corrective actions are required for ${creditMatch.credit_code}.`,
    ...
    return {
      directAnswer: `The highest impact action for ${creditMatch.credit_code} is: ${topAction.action}`,
