...
export class AssignmentReasoner {
  public static evaluate(query: string, runtimeContext: any, graphContext: any): ReasoningOutput {
    ...
    if (!creditMatch) {
       ...
       if (roleMatch) {
           return {
              directAnswer: `The ${roleMatch} role is assigned based on their specific domain expertise for the documentation required.`,
       ...
       if (runtimeContext.project) {
         return {
           directAnswer: "I have analyzed the project context for assignments.",
      ...
      return {
        directAnswer: "I cannot determine which credit or assignment you are asking about.",
    ...
    if (assignments.length === 0) {
      return {
        directAnswer: `For ${creditMatch.credit_code}, the credit is broadly assigned to ${creditMatch.responsible_role || "a single owner"}.`,
    ...
    if (relevantAssignments.length === 0) {
      return {
        directAnswer: `The ${specificRole || "specified role"} does not have explicit assignments for ${creditMatch.credit_code}.`,
    ...
    return {
      directAnswer: `For ${creditMatch.credit_code}, the ${specificRole || "assigned team"} is responsible for providing the ${tasks.join(" and ")}.`,
