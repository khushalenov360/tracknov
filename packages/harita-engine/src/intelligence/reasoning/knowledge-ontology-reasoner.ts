...
export class KnowledgeOntologyReasoner {
  public static async evaluate(query: string): Promise<ReasoningOutput> {
    ...
    const fallbackResponse: ReasoningOutput = {
      directAnswer: "I could not identify the specific credit or evidence type in your query.",
    ...
    try {
      ...
      if (isDocumentQuery) {
        if (creditCode) {
          ...
          if (credit) {
            ...
            if (reqDocs.length > 0) {
              return {
                directAnswer: `The required evidence types for ${creditCode} are: ${reqDocs.join(", ")}.`,
        ...
        if (rag) return rag;
        return { ...fallbackResponse, directAnswer: creditCode ? `Credit ${creditCode} not found. Please verify the credit code.` : fallbackResponse.directAnswer };
      }
      ...
      if (isReviewQuery) {
        if (creditCode) {
          ...
          if (credit) {
            ...
            if (criteria.length > 0) {
              return {
                directAnswer: `Review criteria for ${creditCode}:\n` + criteria.join("\n"),
        ...
        if (rag) return rag;
        return { ...fallbackResponse, directAnswer: creditCode ? `No review criteria seeded yet for ${creditCode}.` : fallbackResponse.directAnswer };
      }
      ...
      if (q.includes("who is responsible") || q.includes("who uploads") || q.includes("who creates") || q.includes("responsible for")) {
        if (evidenceType) {
          ...
          if (evData) {
            ...
            if (roles.length > 0) {
              return {
                directAnswer: `${justNames.join(", ")} is responsible for ${evidenceType}.`,
      ...
      if (q.includes("which credits use") && evidenceType) {
        ...
        if (evData) {
          ...
          return {
            directAnswer: creditCodes.length > 0
      ...
      if (q.includes("which documents is") || q.includes("documents is architect") || q.includes("responsible for uploading")) {
        ...
        if (roleRow) {
          ...
          if (evNames.length > 0) {
            return {
              directAnswer: `${roleName} is responsible for: ${evNames.join(", ")}.`,
        ...
        return rag ?? { ...fallbackResponse, directAnswer: `${roleName} has no mapped document responsibilities.` };
      }
