  ...
  public static evaluate(query: string, runtimeContext: any, graphContext: any): ReasoningOutput {
    ...
    if (!creditMatch) {
      if (runtimeContext.project) {
        return {
          directAnswer: "I have analyzed the project context for risks.",
      ...
      return {
        directAnswer: "I cannot determine which credit you want me to analyze for risks.",
    ...
    if (risks.length === 0) {
      return {
        directAnswer: `There are no significant risks detected for ${creditMatch.credit_code} at this time.`,
    ...
    return {
      directAnswer: `The highest risk for ${creditMatch.credit_code} is: ${highestRisk.detail}`,
