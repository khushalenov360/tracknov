...
import type { ExecutiveAction } from "./executive-prioritization-engine";

export interface DecisionIntelligenceInput {
  certificationGap?: CertificationGap;
  evidenceGaps?: EvidenceGap[];
  workloads?: ContributorWorkload[];
...
export class DecisionIntelligenceEngine {
  ...
  public static evaluate(input: DecisionIntelligenceInput): DecisionIntelligenceResult {
  ...
  private static buildOptions(input: DecisionIntelligenceInput): DecisionOption[] {
    ...
    for (const code of input.certificationGap?.highestRiskCredits || []) {
  ...
  private static rank(options: DecisionOption[]): DecisionOption[] {
  ...
  private static buildComparisons(
  ...
  private static buildReasoning(ranked: DecisionOption[], winner: DecisionOption | null): string {
    if (!winner) {
    ...
    const lines: string[] = [];
    lines.push(
      `The highest-priority action is: "${winner.title}" (ROI Score: ${winner.roiScore}).`
    );
    lines.push(winner.rationale ?? "");

    if (ranked.length > 1) {
      lines.push(
        `Compared to the next option "${ranked[1].title}" (ROI Score: ${ranked[1].roiScore}), ` +
