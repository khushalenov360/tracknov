import { RuntimeContext } from "../lib/runtime/runtime-context-assembler";

export class ProjectIntelligenceEngine {
  public static analyzeProjectHealth(context: RuntimeContext) {
    if (!context.project) return { health: "UNKNOWN", message: "No active project" };
    
    const total = context.credits.length;
    if (total === 0) return { health: "GOOD", message: "No credits tracked." };
