...
import { runInReplayMode } from "./governanceMutationInterceptor";
import { governanceTelemetry } from "./governanceObservabilityBus";
...
export async function executeGovernedReplayHarness(
  ...
  try {
    ...
    const result = await runInReplayMode(projectId, async () => {
      ...
      const currentHash = generateLineageHash(replayData.reconstructedState);
...
async function emitGovernanceEvent(params: any) {
    const { emitGovernanceEvent: emit } = await import("./governanceObservabilityBus");
    return emit(params);
}
