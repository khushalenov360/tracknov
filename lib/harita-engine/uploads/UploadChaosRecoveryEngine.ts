export interface UploadFailureContext {
  ...
  fileSize: number;
  mimeType: string;
  failureReason: "partial_upload" | "network_interrupted" | "encrypted" | "malformed_format" | "rotated_scan" | "corrupted_bytes" | "mobile_disconnect" | "timeout";
  attempts: number;
  lastChunkIndex: number;
  totalChunks: number;
}

export interface IngestionRecoveryResult {
  recovered: boolean;
  ocrQualityScore: number; // 0 to 1
  partialRecoveryPercentage: number; // 0 to 100
  recoveredTextLength: number;
  autoRotated: boolean;
  scannedTablesExtracted: number;
  quarantineRequired: boolean;
  suggestions: string[];
}

export class UploadChaosRecoveryEngine {
  /**
   * Evaluates upload failure logs and determines automatic recovery routes
   */
  static evaluateFailure(context: UploadFailureContext): {
    shouldRetry: boolean;
