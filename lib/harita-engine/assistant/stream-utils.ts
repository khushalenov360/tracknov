...
import { ResponseIntegrityMonitor } from "../runtime/response-integrity-monitor";

export function createResponseStream(textStream: ReadableStream<Uint8Array>, navigateTo?: string, queryId?: string) {
  ...
  ResponseIntegrityMonitor.initializeResponse(monitoredId);
    ...
    async start(controller) {
      ...
      try {
        while (true) {
          ...
          if (done) {
            ResponseIntegrityMonitor.markComplete(monitoredId, fullResponse);
            controller.close();
            break;
          }
          if (value) {
            ...
            ResponseIntegrityMonitor.logChunk(monitoredId, chunkText.length);
