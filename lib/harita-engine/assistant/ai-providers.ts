...
import { type AssistantContext, type AssistantMessage, buildAssistantSystemPrompt } from "@/lib/harita-engine/assistant";
import { toGeminiTools, toOpenAiTools } from "@/lib/harita-engine/assistant-tools";
import {
  type AiProvider,
...
export async function callGeminiWithTools(
  ...
  if (!response.ok) {
    console.error(`[Assistant] Gemini tool detection failed: HTTP ${response.status} - ${await response.text().catch(() => "no body")}`);
    return null;
  }
...
export async function callOpenAiWithTools(
  ...
  if (!response.ok) {
    console.error(`[Assistant] openAiCompatible (${attempt.provider}) failed: HTTP ${response.status} - ${await response.text().catch(() => "no body")}`);
    return null;
  }
