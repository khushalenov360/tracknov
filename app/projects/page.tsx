...
export default async function ProjectsPage({ searchParams }: { searchParams?: Promise<{ error?: string, tab?: string }> }) {
  ...
  return (
    ...
    >
      ...
      {isL3OrAbove ? (
        <section className="surface-card p-6 mb-6 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-b from-blue-100/50 to-transparent blur-3xl pointer-events-none rounded-full" />
          
          <div className="flex flex-wrap items-center justify-between gap-4 relative z-10">
            <div>
              <div className="flex items-center gap-2">
                <span className="h-2 w-2 rounded-full bg-blue-500 animate-ping" />
                <h2 className="text-sm font-black uppercase tracking-wider text-[var(--color-text-primary)]">
            ...
            <Button asChild className="h-[34px] rounded-xl px-4 text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-md transition-transform hover:scale-[1.02] active:scale-[0.98]">
              <Link href="/review-queue">Open validation queue</Link>
            </Button>
          </div>

          <div className="mt-4 overflow-x-auto rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] relative z-10 shadow-sm">
            <table className="min-w-full border-collapse text-xs text-[var(--color-text-primary)]">
              <thead>
                <tr className="border-b border-[var(--color-border)] bg-[var(--color-surface-2)]">
                  {["Project", "Project Code", "Progress", "Pending", "Rejections", "Readiness"].map((heading) => (
                    <th key={heading} className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">
                      {heading}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-[var(--color-border)]">
                {projects.map((project) => {
                  ...
                  return (
                    <tr key={project.id} className="hover:bg-[var(--color-surface-2)] transition-colors group/row">
                      <td className="px-4 py-3 font-semibold">
                        <Link href={`/projects/${project.id}`} className="text-[var(--color-text-primary)] hover:text-blue-600 transition-colors flex items-center gap-1.5">
                          <span>{project.name}</span>
                        </Link>
                      </td>
                      <td className="px-4 py-3">
                        <code className="rounded-md bg-[var(--color-surface-2)] border border-[var(--color-border)] px-2 py-0.5 text-xs font-mono text-[var(--color-text-secondary)]">
                          {project.projectCode}
                        </code>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="relative w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden border border-gray-300">
                            <div 
                              className={`h-full rounded-full bg-gradient-to-r ${ready ? 'from-emerald-500 to-teal-400' : 'from-blue-500 to-blue-400'}`}
                              style={{ width: `${project.overallCompletion}%` }}
                            />
                          </div>
                          <span className="font-mono font-bold text-[var(--color-text-secondary)] text-xs">
                      ...
                      <td className="px-4 py-3 font-mono font-bold">
                        {pendingValidation > 0 ? (
                          <span className="px-2 py-0.5 rounded bg-amber-100 border border-amber-200 text-amber-700 font-bold text-[10px]">
                            {pendingValidation}
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3 font-mono font-bold">
                        {rejections > 0 ? (
                          <span className="px-2 py-0.5 rounded bg-rose-100 border border-rose-200 text-rose-700 font-bold text-[10px]">
                            {rejections}
                          </span>
                        ) : (
                          <span className="text-gray-400">—</span>
                      ...
                      <td className="px-4 py-3">
                        <span 
                          className={`px-2 py-1 rounded-lg border text-[10px] font-bold uppercase tracking-wider ${
                            ready
