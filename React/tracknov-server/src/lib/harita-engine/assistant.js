...
exports.injectSystemRules = injectSystemRules;
exports.buildAssistantSystemPrompt = buildAssistantSystemPrompt;
exports.buildFallbackAssistantReply = buildFallbackAssistantReply;
...
function buildPersonaPrefix(role) {
    if (!role)
        return "";
...
function buildAssistantSystemPrompt(context, workspaceSnapshot, role) {
    var _a;
    const lines = [
        ...
        "Ground your answers in the workspace snapshot, but you may use your general green building and IGBC certification knowledge to explain concepts, guide the user on documentation requirements, and provide advice.",
        "ONUS OF CONTEXT (MANDATORY): You are forbidden from saying you have no context, that you cannot access files, or giving blind refusals. If you feel you are missing details or context, you must proactively list what you DO see in the workspace snapshot, state exactly what is missing, and ask the user to confirm or clarify before proceeding.",
        ...
        `Current item: ${(_a = context.currentItem) !== null && _a !== void 0 ? _a : "none"}`,
        "Facts:",
    ...
    if (workspaceSnapshot === null || workspaceSnapshot === void 0 ? void 0 : workspaceSnapshot.trim()) {
        lines.push("");
...
function buildFallbackAssistantReply(context, prompt) {
    var _a;
    const normalized = prompt.toLowerCase().trim();
    // Handle greetings naturally (Phase 3)
    const isGreeting = normalized === "hi" ||
        normalized === "hello" ||
        normalized === "hey" ||
        normalized === "hi there" ||
    ...
    if (normalized.includes("what can you do") || normalized.includes("how can you help") || normalized.includes("what do you do")) {
        const highestRisk = (_a = context.nextSteps[0]) !== null && _a !== void 0 ? _a : "Unknown";
        return [
