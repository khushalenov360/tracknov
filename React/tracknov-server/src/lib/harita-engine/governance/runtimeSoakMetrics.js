...
exports.aggregateSoakMetrics = aggregateSoakMetrics;
exports.persistSoakMetrics = persistSoakMetrics;
 ...
 */
function aggregateSoakMetrics(projectId) {
    return __awaiter(this, void 0, void 0, function* () {
        const admin = (0, admin_1.createAdminClient)();
        const now = new Date();
        const fiveMinsAgo = new Date(now.getTime() - 5 * 60 * 1000).toISOString();
        // 1. Replay Drift (0% expected)
        const { data: driftData } = yield admin
            .from("replay_certificates")
            .select("deterministic_match")
            .eq("project_id", projectId)
            .gt("created_at", fiveMinsAgo);
        const totalReplays = (driftData === null || driftData === void 0 ? void 0 : driftData.length) || 0;
        const failedMatches = (driftData === null || driftData === void 0 ? void 0 : driftData.filter(d => !d.deterministic_match).length) || 0;
        const replayDriftRate = totalReplays > 0 ? (failedMatches / totalReplays) * 100 : 0;
        // 2. Trace Collisions
        const { count: collisions } = yield admin
        ...
        const { count: activeLocks } = yield admin
            .from("replay_locks")
            .select("*", { count: 'exact', head: true })
            .eq("project_id", projectId);
        const { count: leakedLocks } = yield admin
            .from("governance_incidents")
            .select("*", { count: 'exact', head: true })
            .eq("project_id", projectId)
...
function persistSoakMetrics(projectId, metrics) {
    return __awaiter(this, void 0, void 0, function* () {
        const admin = (0, admin_1.createAdminClient)();
        yield admin.from("runtime_metrics").insert({
