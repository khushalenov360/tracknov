...
import { IgbcScoreAuthority } from "./igbc-score-authority";
import { ChillerExtraction, RainwaterExtraction, MaterialExtraction } from "../compliance-schemas/igbc-manifests";

export interface ProjectMetadataBlock {
  projectId: string;
  tenantId?: string;
  projectPath?: string;
}

export function evaluateStructuredCredit(
  creditType: "CHILLER" | "RAINWATER" | "MATERIAL", 
  projectMetadata: ProjectMetadataBlock,
  data: any
) {
  switch (creditType) {
    case "CHILLER":
      return IgbcScoreAuthority.verifyChillerEfficiency(data as ChillerExtraction, projectMetadata);
    case "RAINWATER":
      return IgbcScoreAuthority.verifyRainwaterHarvesting(data as RainwaterExtraction, projectMetadata);
    case "MATERIAL":
      return IgbcScoreAuthority.verifyRecycledContentValue(data as MaterialExtraction[], projectMetadata);
    default:
