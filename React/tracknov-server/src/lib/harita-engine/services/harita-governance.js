...
exports.requiresToolCall = requiresToolCall;
exports.sanitizeAiResponse = sanitizeAiResponse;
exports.filterTechnicalLeakage = filterTechnicalLeakage;
exports.containsAuthoritativeClaim = containsAuthoritativeClaim;
exports.getAuthoritativeClaimRefusal = getAuthoritativeClaimRefusal;
exports.classifyAttachmentIntent = classifyAttachmentIntent;
exports.semanticDisambiguateIntent = semanticDisambiguateIntent;
exports.getUnknownDataResponse = getUnknownDataResponse;
exports.sanitizeUserText = sanitizeUserText;
exports.sanitizeContextText = sanitizeContextText;
exports.routeHaritaIntent = routeHaritaIntent;
exports.normalizeHaritaResponse = normalizeHaritaResponse;
exports.requiresExplicitConfirmationForExecution = requiresExplicitConfirmationForExecution;
// ─────────────────────────────────────────────
 ...
 */
function requiresToolCall(category) {
 ...
 */
function filterTechnicalLeakage(text) {
    let result = text;
 ...
 */
function containsAuthoritativeClaim(text) {
    return AUTHORITATIVE_CLAIM_PATTERNS.some((pattern) => pattern.test(text));
}
function getAuthoritativeClaimRefusal() {
 ...
 */
function classifyAttachmentIntent(text) {
    const q = text.toLowerCase().trim();
    const hasExplicitUpload = (q.includes("upload") || q.includes("map this to") || q.includes("submit this file") || q.includes("push to workflow")) &&
        (q.includes("confirm") || q.includes("yes upload") || q.includes("proceed upload") || q.includes("and upload"));
    const hasAnalysisIntent = q.includes("analyz") || q.includes("analyse") || q.includes("explain") ||
...
// ─────────────────────────────────────────────
function semanticDisambiguateIntent(query, recentContext = "") {
    const q = query.toLowerCase().trim();
    const context = recentContext.toLowerCase();
    // Workflow actions requiring tool execution
    if (q.includes("upload") && (q.includes("confirm") || q.includes("proceed") || q.includes("yes")))
        return "workflow_action";
    if (q.includes("map this") || q.includes("submit this") || q.includes("push to workflow"))
        return "workflow_action";
    // Document analysis
    if (q.includes("read this file") || q.includes("what is this") || q.includes("tell me about the attachment"))
        return "analyze_document";
    if (q.includes("compare"))
        return "document_comparison";
    // Memory-aware followup
    if (q.includes("what did you find") || q.includes("tell me more") || q.includes("elaborate") || (q.includes("what about") && context.includes("analys")))
        return "followup_analysis";
    // Certification Strategy & Gap Analysis
    if (q.includes("how close are we") || q.includes("route to gold") || q.includes("route to platinum") || q.includes("point") || q.includes("strategy"))
        return "certification_strategy";
    if (q.includes("gap") || q.includes("missing") || q.includes("block"))
        return "gap_analysis";
    if (q.includes("ready") || q.includes("can we submit"))
        return "submission_readiness";
    // Credit Guidance
    if (q.includes("applicable") || q.includes("valid for") || q.includes("support eda") || q.includes("help with"))
        return "credit_guidance";
    // Status and Clarifications
    if (q.includes("status") || q.includes("progress"))
        return "project_status";
    if (q.includes("clarif"))
        return "clarification_response";
    // Knowledge and General
    if (q.includes("how to") || q.includes("what is igbc"))
...
function getUnknownDataResponse() {
...
function sanitizeContextText(input) {
    return input
...
function routeHaritaIntent(query) {
    const q = query.toLowerCase();
    if (q.includes("who owns") || q.includes("who is responsible") || q.includes("list contributors") || q.includes("who is working on"))
        return "MULTI_CONTRIBUTOR_QUERY";
    if (q.includes("how many pending") || q.includes("total counts") || q.includes("overall status"))
        return "status";
    if (q.includes("check validation") || q.includes("is it compliant") || q.includes("validation rules"))
        return "validation";
    if (q.includes("workflow status") || q.includes("show my workflow") || q.includes("status of my documents"))
        return "workflow";
    if (q.includes("map this file to") || q.includes("tag this as") || q.includes("upload as document type"))
        return "mapping";
    if (q.includes("summarize this") || q.includes("analyze this file") || q.includes("tell me about the attachment"))
        return "summary";
    if (q.includes("compare these") || q.includes("vs credit"))
        return "comparison";
    if (q.includes("what is my next step") || q.includes("what should i do next"))
        return "next_step";
    return "general";
}
function normalizeHaritaResponse(input) {
    var _a, _b, _c, _d, _e;
    const normalized = {
        assessment: ((_a = input.assessment) !== null && _a !== void 0 ? _a : "").trim() || UNKNOWN_DATA_RESPONSE,
        fit: (_b = input.fit) !== null && _b !== void 0 ? _b : "Medium",
        reason: ((_c = input.reason) !== null && _c !== void 0 ? _c : "").trim() || UNKNOWN_DATA_RESPONSE,
        recommendation: ((_d = input.recommendation) !== null && _d !== void 0 ? _d : "").trim() || "Please share one concrete project detail so I can guide the next valid step.",
        confirm: ((_e = input.confirm) !== null && _e !== void 0 ? _e : "").trim() || "Confirm?",
    };
    return [
...
function requiresExplicitConfirmationForExecution(query, options) {
    if ((options === null || options === void 0 ? void 0 : options.analysisOnly) === true)
        return false;
    const q = query.toLowerCase();
    if (q.includes("who uploads") || q.includes("who is responsible") || q.includes("what") || q.includes("how") || q.includes("can we submit") || q.includes("be submitted"))
        return false;
