...
import { headroomCompressorService } from "./headroom-compressor";
...
async function generateEmbedding(text: string): Promise<number[]> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[RAG] Embedding failed: HTTP ${response.status} - ${await response.text().catch(()=>"")}`);
      return vector;
    }
...
export class RAGService {
  ...
  private async upsertChunks(documentId: string | null, chunks: RAGChunk[]) {
    ...
    const rows = await Promise.all(chunks.map(async (chunk) => ({
      ...
      embedding: toVectorLiteral(await generateEmbedding(chunk.content)),
  ...
  async ingestGuidebookPdf(params: {
    ...
    const ccrResult = await headroomCompressorService.compressReferenceGuide(cleanedText, params.guidebookId);
