...
export class ProjectService {
  ...
  private async instantiateProjectCreditsIfMissing(projectId: string, ratingSystemId?: string | null): Promise<number> {
    if (!ratingSystemId) return 0;

    const { data: existingCredits } = await this.admin
    ...
    if (missingTemplates.length > 0) {
      const projectCreditsToInsert = missingTemplates.map((template: any) => ({
  ...
  async uploadProjectDataTable(user: CurrentUser, params: {
    projectId: string;
    file: File;
    title?: string;
  }) {
    ...
    const safeTitle = (params.title ?? params.file.name.replace(/\.[^.]+$/, "")).trim().slice(0, 200) || "Data Table";
    ...
    const filePath = `${params.projectId}/datatables/${Date.now()}-${randomUUID()}-${sanitizedBase}${lowerName.substring(lowerName.lastIndexOf('.'))}`;

    const { data: existingDataTable } = await this.admin
    ...
    if (existingDataTable?.id) {
      ...
      if (!error && existingDataTable.file_path && existingDataTable.file_path !== filePath) {
        await this.admin.storage.from("project-documents").remove([existingDataTable.file_path]);
      }
    } else {
      const { error } = await this.admin.from("project_data_tables").insert(writePayload);
      writeError = error;
    }
