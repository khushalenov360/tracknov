...
exports.executiveConsistencyValidator = exports.ExecutiveConsistencyValidator = void 0;
// ---------------------------------------------------------------------------
...
// Main Validator
// ---------------------------------------------------------------------------
class ExecutiveConsistencyValidator {
    /**
    ...
    validate(input) {
        ...
        // Severity: CRITICAL
        // -----------------------------------------------------------------------
        const certGoldSecured = ((_a = input.certification) === null || _a === void 0 ? void 0 : _a.goldSecured) === true ||
            (input.certification !== undefined &&
                ((_b = input.certification.narrative) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes("gold is already secured")));
        const certGoldAtRisk = ((_c = input.risk) === null || _c === void 0 ? void 0 : _c.goldAtRisk) === true ||
            (input.certification !== undefined &&
                input.certification.riskPoints > 0 &&
                ((_d = input.certification.narrative) === null || _d === void 0 ? void 0 : _d.toLowerCase().includes("gold becomes unattainable")));
        if (certGoldSecured && certGoldAtRisk) {
            violations.push({
                ...
                explanation: "The certification engine reports Gold as secured while the risk engine reports it remains at risk. " +
        ...
        // Severity: HIGH
        // -----------------------------------------------------------------------
        const zeroRiskPoints = input.certification !== undefined && input.certification.riskPoints === 0;
        const briefHasRisks = Array.isArray((_e = input.executiveBrief) === null || _e === void 0 ? void 0 : _e.risks) &&
        ...
        if (zeroRiskPoints && briefHasRisks) {
            violations.push({
                ...
                explanation: "The certification engine shows no points at risk, but the executive brief enumerates active risks. " +
        ...
        // Severity: HIGH
        // -----------------------------------------------------------------------
        const submissionReady = ((_f = input.readiness) === null || _f === void 0 ? void 0 : _f.submissionReady) === true;
        const hasBlockedCredits = typeof ((_g = input.risk) === null || _g === void 0 ? void 0 : _g.blockedCredits) === "number" &&
        ...
        if (submissionReady && hasBlockedCredits) {
            violations.push({
                ...
                explanation: "A submission cannot be ready while one or more credits remain blocked. " +
        ...
        // Severity: MEDIUM
        // -----------------------------------------------------------------------
        const noMissingDocs = typeof ((_h = input.evidence) === null || _h === void 0 ? void 0 : _h.missingDocuments) === "number" &&
            input.evidence.missingDocuments === 0;
        const hasMissingCredits = typeof ((_j = input.evidence) === null || _j === void 0 ? void 0 : _j.missingCredits) === "number" &&
        ...
        if (noMissingDocs && hasMissingCredits) {
            violations.push({
                ...
                explanation: "The document-level count reports zero missing items, but the credit-level analysis " +
        ...
        for (const credit of input.credits || []) {
            if (credit.complete && ((_k = credit.rejectedDocuments) !== null && _k !== void 0 ? _k : 0) > 0) {
                violations.push({
                    ...
                    explanation: `Credit ${credit.credit_code} cannot be complete while it holds rejected evidence. ` +
        ...
        // Severity: MEDIUM
        // -----------------------------------------------------------------------
        const noConstraint = ((_l = input.workload) === null || _l === void 0 ? void 0 : _l.noConstraint) === true;
        const overloadedCount = (_p = (_o = (_m = input.workload) === null || _m === void 0 ? void 0 : _m.overloadedUsers) === null || _o === void 0 ? void 0 : _o.length) !== null && _p !== void 0 ? _p : 0;
        if (noConstraint && overloadedCount > 0) {
            violations.push({
                ...
                explanation: "The workload summary claims no resource constraints, but the detailed workload analysis " +
        ...
        return Object.assign({ passed: violations.length === 0, violations,
            confidence }, (criticalCount > 0 ? { _criticalCount: criticalCount } : {}));
    }
    /**
     * Returns the suppression message when the validator blocks a response.
     */
    static suppressionMessage(report) {
        const criticalCodes = report.violations
            .filter((v) => v.severity === "CRITICAL")
            .map((v) => v.code)
            .join(", ");
        return ("I detected conflicting project intelligence signals while preparing this recommendation.\n\n" +
...
exports.ExecutiveConsistencyValidator = ExecutiveConsistencyValidator;
exports.executiveConsistencyValidator = new ExecutiveConsistencyValidator();
