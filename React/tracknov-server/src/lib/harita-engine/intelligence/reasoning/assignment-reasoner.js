...
exports.AssignmentReasoner = void 0;
...
class AssignmentReasoner {
    static evaluate(query, runtimeContext, graphContext) {
        ...
        if (specificRole) {
            relevantAssignments = assignments.filter((req) => (req.contributorName || "").toLowerCase().includes(specificRole) ||
        ...
        const tasks = relevantAssignments.map((req) => req.requirementType);
        const explanation = explanation_engine_1.ExplanationEngine.explainAssignment({
        ...
        return {
            ...
            evidence: relevantAssignments.map((req) => `- ${req.requirementType} is assigned to ${req.contributorName || "Unassigned"}`).join("\n"),
...
exports.AssignmentReasoner = AssignmentReasoner;
