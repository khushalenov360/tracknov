...
const reviewerPersona_1 = require("./reviewerPersona");
function buildReviewerPrompt(context) {
    ...
    const systemPrompt = `${reviewerPersona_1.reviewerPersona}
...
- Do not produce open-ended narrative paragraphs before the status line or before the matrix.`;
    ...
    const briefingLines = [];
    if (!securityApproved) {
        briefingLines.push(`[Security Alert]: ${(_a = facts.member_access_error) !== null && _a !== void 0 ? _a : "Access strictly denied due to permission rules."}`);
    }
    else {
        if (facts.credit) {
        ...
        if (facts.documents) {
            ...
            if (facts.documents.length === 0) {
                ...
                for (const document of facts.documents) {
                    briefingLines.push(`${document.file_name} [${(_b = document.doc_category) !== null && _b !== void 0 ? _b : "uncategorised"}] - ${(_c = document.workflow_state) !== null && _c !== void 0 ? _c : "unknown state"}`);
                }
            }
        }
        if (facts.members) {
    ...
    const userMessage = [
        ...
        "",
        `User Name: ${userName}`,
        `User Role: ${userRole !== null && userRole !== void 0 ? userRole : "unknown"}`,
        ...
        `Classified Module Category: ${(_d = plan.module_category) !== null && _d !== void 0 ? _d : "unclassified"}`,
        briefingLines.join("\n"),
        `</project_database_current_state>`,
        "",
