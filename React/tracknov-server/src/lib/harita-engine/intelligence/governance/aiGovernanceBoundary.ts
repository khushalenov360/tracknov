 ...
 */
export class AiGovernanceBoundary {
  /**
   * Validates that an AI recommendation does not attempt to mutate authoritative state.
   */
  static validateRecommendation(action: string, payload: any) {
    ...
    if (payload?.authoritative === true || payload?.skipValidation === true) {
   ...
   */
  static validateFrameworkAlignment(recommendationFramework: string, projectFramework: string) {
