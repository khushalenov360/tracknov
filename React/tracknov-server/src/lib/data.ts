...
export const getDashboardProjects = cache(async function getDashboardProjects(): Promise<ProjectSummary[]> {
  ...
  const summaries = projectRows.map((project: any) => {
    ...
    const getDocumentScore = (state: string | null | undefined): number => {
      if (!state) return 0;
      const upper = state.toUpperCase();
      if (upper === "APPROVED") return 6;
      if (["UNDER_REVIEW", "CLARIFICATION", "REJECTED", "RESUBMITTED"].includes(upper)) return 5;
      if (["SUBMITTED", "READY"].includes(upper)) return 3;
      return 0;
    };
    ...
    credits.forEach((credit: any) => {
      const requiredTypes = new Set(
        (credit.documents_required || [])
          .filter((req: any) => req.required && req.type)
          .map((req: any) => req.type)
      );
      let n = requiredTypes.size;
      const creditDocs = documents.filter((d: any) => d.project_credit_id === credit.id);
      
      if (n === 0 && creditDocs.length > 0) {
        n = creditDocs.length;
        totalPoints += creditDocs.reduce((acc: number, doc: any) => acc + getDocumentScore(doc.state), 0);
      } else if (n === 0) {
        ...
        Array.from(requiredTypes).forEach((type) => {
          const matchingDocs = creditDocs.filter((d: any) => String(d.doc_category ?? "").trim() === String(type).trim());
          if (matchingDocs.length > 0) {
            totalPoints += Math.max(...matchingDocs.map((d: any) => getDocumentScore(d.state)));
          }
        });
      }
      totalN += n;
    ...
    categories: Object.entries(categoryMeta).map(([key, meta]) => {
      ...
      categoryCredits.forEach((credit) => {
        ...
        if (n === 0 && uploadedDocs.length > 0) {
          ...
          totalPoints += uploadedDocs.reduce((acc: number, doc: any) => acc + getDocumentScore(doc.state), 0);
        } else if (n === 0) {
          if (String(credit.status).toUpperCase() === "COMPLETE") {
            n = 1;
            totalPoints += 6;
          ...
          Array.from(requiredTypes).forEach((type) => {
            const matchingDocs = uploadedDocs.filter((d: any) => String(d.doc_category ?? "").trim() === String(type).trim());
            if (matchingDocs.length > 0) {
              totalPoints += Math.max(...matchingDocs.map((d: any) => getDocumentScore(d.state)));
            }
          });
  ...
  if ((currentUser?.role === "super_user" || currentUser?.role === "L5") && env.supabaseServiceRoleKey) {
    ...
    (profiles ?? []).forEach((profile: any) => {
      grouped.set(profile.user_id, {
        ...
        token_balance:
          normalizeRole(profile.global_role) === "client"
            ? walletByClient.get(profile.user_id) ?? 0
            : undefined,
