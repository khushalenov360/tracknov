export interface CreditMappingSuggestion {
  bimElementId: string;
  elementClass: string;
  targetCreditCode: string;
  matchScore: number;
  evidenceCandidateName: string;
}

export class BimEntityMapper {
  /**
   * Translates extracted model components into specific documentation candidates
   */
  static mapEntitiesToCredits(elements: { id: string; className: string }[]): CreditMappingSuggestion[] {
