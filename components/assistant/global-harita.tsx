...
import { sessionMemory } from "@/lib/harita-engine/services/session-memory-service";
...
export function GlobalHarita({ enabled, role, title, description, persistent }: GlobalHaritaProps) {
  ...
  async function sendPrompt(prompt: string) {
    ...
    try {
      let currentAttachmentFile = attachmentFile;
      let currentAttachment = attachment;
      
      const lowerText = text.toLowerCase();
      if (lowerText.includes("discard") || lowerText.includes("clear image") || lowerText.includes("remove file") || lowerText.includes("stop analy")) {
      ...
      if (currentAttachmentFile && isAnalysisPrompt(text)) {
